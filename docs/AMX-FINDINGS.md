...
> **Internal engineering record — not booth material and not linked from the app.**
> The shipped demo is AVX-512 only and says nothing about AMX anywhere on screen.
> This document is kept because it is the measured evidence *behind* that decision:
> it is why there is no AMX claim to defend. Some commands and config keys quoted
> below (`amx.use_bf16_injection`, `app/container_inject/`, the AMX race mode) have
> since been removed from the codebase; they are preserved here as a record of how
